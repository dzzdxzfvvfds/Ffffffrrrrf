...
class GoogleSheetsSyncRequest(BaseModel):
    ...
    # Nuova struttura: scelte complete con azioni Gestisci
    conflict_actions: Optional[dict] = None  # {name: {action: 'selected'|'not_selected'|'replace'|'create_new', replace_with: ..., replace_with_id: ...}}
...
class SyncChoice(BaseModel):
    """Traccia le scelte fatte durante ogni sincronizzazione"""
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    ambulatorio: str
    sync_id: str  # ID della sincronizzazione
    name: str  # Nome dal foglio
    action: str  # 'selected', 'not_selected', 'replace', 'create_new'
    replace_with: Optional[str] = None  # Nome del paziente con cui sostituire (se action='replace')
    replace_with_id: Optional[str] = None  # ID paziente per 'replace'
    dates: List[str] = []  # Date in cui compare questo nome
    created_at: str = Field(default_factory=lambda: datetime.now(timezone.utc).isoformat())
    created_by: str = ""

class AnalyzedAppointment(BaseModel):
    """Traccia gli appuntamenti già analizzati per non rianalizzarli"""
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    ambulatorio: str
    patient_name: str  # "cognome nome" normalizzato
    date: str  # YYYY-MM-DD
    ora: str
    tipo: str  # PICC/MED
...
async def sync_from_google_sheets(
    ...
    NUOVA LOGICA:
    - conflict_actions contiene le azioni per ogni nome con priorità Gestisci > Flag
    - I nomi non selezionati E senza azione Gestisci vengono salvati come "not_selected"
    - Le vecchie "not_selected" vengono eliminate e sostituite con le nuove
    - Gli appuntamenti sincronizzati vengono marcati come "analizzati"
    """
    if data.ambulatorio.value not in payload["ambulatori"]:
        raise HTTPException(status_code=403, detail="Non hai accesso a questo ambulatorio")
    ...
    # Estrai le azioni dai conflitti - priorità Gestisci > Flag
    conflict_actions = data.conflict_actions or {}
    ...
    try:
        ...
        for apt in existing_appointments:
            # Usa FULL NAME per match più robusto
            cognome = apt.get("patient_cognome", "").strip()
            nome = apt.get("patient_nome", "").strip()
            full_name = f"{cognome} {nome}".strip().lower()
            key = (
                full_name,
                apt.get("data", ""),
                apt.get("ora", ""),
        ...
        for apt in all_appointments:
            # Usa FULL NAME per match più robusto
            cognome = apt["cognome"].strip() if apt["cognome"] else ""
            nome = apt["nome"].strip() if apt["nome"] else ""
            full_name = f"{cognome} {nome}".strip().lower()
            key = (
                full_name,
                apt["date"],
        ...
        # NUOVA LOGICA: Applica conflict_actions (Gestisci > Flag)
        # conflict_actions = {name: {action: 'selected'|'not_selected'|'replace'|'create_new', replace_with: ..., replace_with_id: ...}}
        names_to_skip = set()  # Nomi da ignorare (not_selected senza azione gestisci)
        name_replacements = {}  # Nomi da sostituire: old_name -> new_name
        names_to_create = set()  # Nomi da creare come nuovi pazienti
        
        if conflict_actions:
            logger.info(f"Applicando {len(conflict_actions)} azioni dai conflitti")
            
            for name, action_data in conflict_actions.items():
                action = action_data.get("action", "not_selected")
                
                if action == "not_selected":
                    # Nome da ignorare - salva nel database scelte
                    ...
                    replace_with = action_data.get("replace_with", "")
                    replace_with_id = action_data.get("replace_with_id", "")
                    if replace_with:
                        name_replacements[name.lower().strip()] = {
                            "name": replace_with,
                            "id": replace_with_id
                        }
        ...
        if name_corrections:
            logger.info(f"Applicando {len(name_corrections)} correzioni nomi legacy")
            ...
            for apt in new_appointments_to_process:
                ...
                if full_name in name_corrections:
                    ...
                    logger.info(f"Corretto (legacy): '{full_name}' -> '{apt['cognome']} {apt['nome']}'")
        
        # Applica le nuove sostituzioni e filtra i nomi da skippare
        filtered_appointments = []
        for apt in new_appointments_to_process:
            full_name = f"{apt['cognome']} {apt['nome']}".strip()
            full_name_lower = full_name.lower().strip()
            
            # Skip se il nome è nella lista da ignorare
            if full_name_lower in names_to_skip:
                logger.debug(f"Skip nome non selezionato: {full_name}")
                continue
            
            # Applica sostituzione se presente
            if full_name_lower in name_replacements:
                replacement = name_replacements[full_name_lower]
                parts = replacement["name"].split(maxsplit=1)
                apt['cognome'] = parts[0]
                apt['nome'] = parts[1] if len(parts) > 1 else ""
                apt['_replace_with_id'] = replacement.get("id")
                logger.info(f"Sostituito: '{full_name}' -> '{apt['cognome']} {apt['nome']}'")
            
            filtered_appointments.append(apt)
        
        new_appointments_to_process = filtered_appointments
        logger.info(f"Appuntamenti dopo filtro azioni: {len(new_appointments_to_process)}")
        
        # STEP 3: Estrai TUTTI i pazienti unici dagli appuntamenti
        all_patient_names = set()
        for apt in new_appointments_to_process:
            all_patient_names.add((apt["cognome"], apt["nome"]))
        
        logger.info(f"Pazienti unici da processare: {len(all_patient_names)}")
        ...
        # Prima carica TUTTI i pazienti esistenti nel DB per questo ambulatorio
        existing_patients_in_db = await db.patients.find(
            {"ambulatorio": data.ambulatorio.value},
            {"id": 1, "cognome": 1, "nome": 1, "_id": 0}
        ).to_list(None)
        
        # Crea mappa per lookup veloce
        for p in existing_patients_in_db:
            key = (p["cognome"], p.get("nome", ""))
            patient_id_map[key] = p["id"]
            # Aggiungi anche versione lowercase per match case-insensitive
            key_lower = (p["cognome"].lower(), (p.get("nome") or "").lower())
            if key_lower not in patient_id_map:
                patient_id_map[key_lower] = p["id"]
        ...
        # 1. Già esistenti nel DB
        # 2. Creati esplicitamente dall'utente (action = 'create_new' o 'replace')
        # 3. Con _replace_with_id già impostato
        
        patients_to_create = []  # Lista di pazienti da creare (solo quelli espliciti)
        
        for cognome, nome in all_patient_names:
            # Skip se già nella mappa
            if (cognome, nome) in patient_id_map:
                continue
            if (cognome.lower(), (nome or "").lower()) in patient_id_map:
                continue
            ...
            if existing:
                ...
                # Controlla le azioni dalla richiesta attuale (conflict_actions dal frontend)
                ...
                if action_for_name and action_for_name.get("action") in ["create_new", "selected"]:
                    ...
                    patient_tipos = set()
                    for apt in new_appointments_to_process:
                        if apt["cognome"].lower() == cognome.lower() and (apt["nome"] or "").lower() == (nome or "").lower():
                    ...
                    created_patients += 1
                    logger.info(f"Creato paziente (esplicito): {cognome} {nome}")
                elif action_for_name and action_for_name.get("action") == "replace" and action_for_name.get("replace_with_id"):
                    ...
                    logger.warning(f"Paziente non trovato e nessuna azione: {cognome} {nome} - appuntamenti skippati")
        
        # STEP 4: Crea SOLO i nuovi appuntamenti
        skipped_no_patient = 0
        skipped_existing = 0
        skipped_slot_limit = 0
        
        for apt in new_appointments_to_process:
            patient_id = patient_id_map.get((apt["cognome"], apt["nome"]))
            if not patient_id:
                # Prova anche con case-insensitive
                for (cog, nom), pid in patient_id_map.items():
                    if cog.lower() == apt["cognome"].lower() and nom.lower() == apt["nome"].lower():
                        patient_id = pid
                        break
            
            if not patient_id:
                skipped_no_patient += 1
                logger.warning(f"Skip appuntamento - paziente non trovato: {apt['cognome']} {apt['nome']}")
            ...
            # Se l'appuntamento ha un replace_with_id, usa quello direttamente
            if apt.get('_replace_with_id'):
                patient_id = apt['_replace_with_id']
            
            # RIMOSSO: Verifica limiti slot - lascia creare tutti gli appuntamenti
            # La verifica dei limiti slot non deve bloccare l'importazione da Google Sheets
            # perché il foglio rappresenta la pianificazione reale
            ...
            # NUOVO: Salva l'appuntamento come "analizzato" per non rianalizzarlo in futuro
            # IMPORTANTE: Normalizza il nome (strip + lower) per match consistente
            normalized_name = f"{apt['cognome'].strip()} {(apt['nome'] or '').strip()}".strip().lower()
            analyzed_apt = AnalyzedAppointment(
                ambulatorio=data.ambulatorio.value,
            ...
            await db.analyzed_appointments.insert_one(analyzed_apt.model_dump())
...
async def analyze_google_sheets_sync(
    ...
    """
    Analizza i dati da Google Sheets - SINCRONIZZAZIONE INCREMENTALE VERA
    
    Logica:
    1. Prende TUTTI gli appuntamenti dal foglio Google
    2. ESCLUDE gli appuntamenti che esistono già nel sistema (collezione appointments)
    3. ESCLUDE gli appuntamenti già analizzati/sincronizzati in precedenza (anche se eliminati manualmente)
       → Questo garantisce che le modifiche manuali abbiano priorità sulla sync
    4. Per i NUOVI appuntamenti, se il paziente esiste già nel DB → associa direttamente
    5. Mostra conflitti per nomi simili/duplicati
    6. Applica scelte precedenti (sync_choices) automaticamente
    """
    if data.ambulatorio.value not in payload["ambulatori"]:
        raise HTTPException(status_code=403, detail="Non hai accesso a questo ambulatorio")
    ...
    try:
        ...
        analyzed_appointments = await db.analyzed_appointments.find(
            {"ambulatorio": data.ambulatorio.value},
            {"patient_name": 1, "date": 1, "ora": 1, "tipo": 1, "_id": 0}
        ).to_list(None)
        
        analyzed_keys = set()
        for aa in analyzed_appointments:
            # Chiave: (nome_paziente, data, ora, tipo)
            key = (aa["patient_name"].lower(), aa["date"], aa["ora"], aa["tipo"])
        ...
        # Crea set per verificare appuntamenti esistenti
        # Usiamo FULL NAME (cognome + nome) per evitare problemi di split diversi
        existing_apt_keys = set()
        existing_apt_by_patient = set()
        for apt in existing_appointments:
            # Key per patient_id
            key1 = (apt.get("patient_id"), apt.get("data"), apt.get("ora"))
            existing_apt_by_patient.add(key1)
            
            # Key per nome paziente - USA FULL NAME normalizzato
            # Rimuovi caratteri speciali come "." che possono essere placeholder
            cognome = (apt.get("patient_cognome") or "").strip()
            nome = (apt.get("patient_nome") or "").strip()
            # Pulisci nomi che sono solo punti o caratteri speciali
            if nome in [".", "-", "_", "/"]:
                nome = ""
            full_name = f"{cognome} {nome}".strip().lower()
            ...
            # Chiave: (full_name, data, ora) - più robusta
            key2 = (full_name, apt.get("data"), apt.get("ora"))
            existing_apt_keys.add(key2)
        
        logger.info(f"Appuntamenti esistenti nel sistema: {len(existing_appointments)}")
        ...
        # STEP 3: Filtra appuntamenti - tieni SOLO quelli NON ancora nel sistema E NON già sincronizzati
        new_appointments = []
        skipped_existing = 0
        ...
        for k in existing_apt_keys:
            if 'trapani' in str(k[0]).lower():
                logger.info(f"DEBUG existing_apt_keys trapani: {k}")
        for k in analyzed_keys:
            if 'trapani' in str(k[0]).lower():
                logger.info(f"DEBUG analyzed_keys trapani: {k}")
        
        for apt in all_appointments:
            # IMPORTANTE: Normalizza i nomi rimuovendo spazi extra e caratteri speciali
            apt_cognome = apt['cognome'].strip() if apt['cognome'] else ""
            apt_nome = (apt['nome'] or "").strip()
            # Pulisci nomi che sono solo punti o caratteri speciali
            if apt_nome in [".", "-", "_", "/"]:
                apt_nome = ""
            
            full_name = f"{apt_cognome} {apt_nome}".strip()
            # Normalizza spazi multipli
            import re
            full_name = re.sub(r'\s+', ' ', full_name)
            full_name_lower = full_name.lower()
            cognome_lower = apt_cognome.lower()
            nome_lower = apt_nome.lower()
            
            # DEBUG per Di Trapani
            if 'trapani' in cognome_lower:
                logger.info(f"DEBUG Processing Di Trapani: cognome='{cognome_lower}', nome='{nome_lower}', full='{full_name_lower}', date={apt['date']}, ora={apt['ora']}, tipo={apt['tipo']}")
                apt_key_debug = (full_name_lower, apt["date"], apt["ora"])
                analyzed_key_debug = (full_name_lower, apt["date"], apt["ora"], apt["tipo"])
                logger.info(f"DEBUG apt_key: {apt_key_debug}, in existing: {apt_key_debug in existing_apt_keys}")
                logger.info(f"DEBUG analyzed_key: {analyzed_key_debug}, in analyzed: {analyzed_key_debug in analyzed_keys}")
            
            # Controlla se questo appuntamento esiste già nel sistema
            # Usa FULL NAME per match più robusto
            apt_key = (full_name_lower, apt["date"], apt["ora"])
            if apt_key in existing_apt_keys:
                skipped_existing += 1
                logger.debug(f"Skip appuntamento già esistente: {full_name} {apt['date']} {apt['ora']}")
                continue
            
            # IMPORTANTE: Controlla se questo appuntamento è già stato sincronizzato in precedenza
            # Se sì, NON lo ri-creiamo (anche se è stato eliminato manualmente)
            # Perché: modifiche manuali > sincronizzazione
            # Uso la stessa struttura di chiave di analyzed_appointments
            analyzed_key = (full_name_lower, apt["date"], apt["ora"], apt["tipo"])
            if analyzed_key in analyzed_keys:
            ...
            if patient_id:
                ...
                if apt_key2 in existing_apt_by_patient:
                    ...
                    skipped_existing += 1
                    logger.debug(f"Skip appuntamento esistente (by patient_id): {full_name} {apt['date']} {apt['ora']}")
                    continue
                else:
        ...
        for apt in new_appointments:
            if "_matched_patient_id" not in apt:
                # Questo paziente non esiste nel DB - sarà mostrato come conflitto
                new_patient_names.add((apt["cognome"], apt["nome"]))
                logger.info(f"Aggiunto a new_patient_names: '{apt['cognome']} {apt['nome']}'")
        
        logger.info(f"Nuovi pazienti (non nel DB): {len(new_patient_names)}")
        logger.info(f"new_patient_names: {list(new_patient_names)[:10]}...")
        
        # STEP 5: Mostra TUTTI i nuovi pazienti come "da gestire"
        # L'utente DEVE decidere manualmente: crea nuovo o sostituisci con esistente
        # NON creare mai pazienti automaticamente
        conflicts = []
        processed_names = set()
        
        # Conta occorrenze per i nuovi nomi
        name_occurrences = {}
        for apt in new_appointments:
            if "_matched_patient_id" not in apt:
                full_name = f"{apt['cognome']} {apt['nome']}".strip()
        ...
        sheet_names_new = set(f"{c} {n}".strip() for c, n in new_patient_names)
        
        # Gruppo 1: Pazienti con nomi simili ad altri (conflitti veri)
        # Gruppo 2: Pazienti NUOVI senza match (devono essere gestiti manualmente)
        
        for cognome, nome in new_patient_names:
            full_name = f"{cognome} {nome}".strip()
            ...
            # Cerca nomi simili per raggruppare eventuali conflitti
            similar_results = find_similar_names(full_name, existing_names, sheet_names_new)
            
            if similar_results:
                # Ha nomi simili - crea conflitto raggruppato
                ...
                # Conta quanti sono nel DB vs nuovi
                names_in_db = [n for n in all_names_in_conflict if n.lower() in existing_names_lower]
                names_new = [n for n in all_names_in_conflict if n.lower() not in existing_names_lower]
                
                # Se tutti i nomi sono già nel DB, skip (pazienti già confermati separati)
                if len(names_new) == 0:
                    continue
                
                has_existing = len(names_in_db) > 0
                
                for name in all_names_in_conflict:
                    occ = name_occurrences.get(name, {"count": 0, "dates": set(), "tipo": set()})
                    exists_in_db = name.lower() in existing_names_lower
                    
                    # Cerca ID paziente se esiste
                    patient_id_for_name = None
                    if exists_in_db:
                        name_parts = name.split(maxsplit=1)
                        cognome_search = name_parts[0].lower() if name_parts else ""
                        nome_search = name_parts[1].lower() if len(name_parts) > 1 else ""
                        patient_id_for_name = existing_patients_by_fullname.get(f"{cognome_search} {nome_search}".strip())
                    ...
                    conflict_options.append({
                        "name": name,
                        "patient_id": patient_id_for_name,
                ...
                conflict_options.sort(key=lambda x: (-int(x["exists_in_db"]), -x["occurrences"], -x["similarity"]))
                
                suggested = next((opt["name"] for opt in conflict_options if opt["exists_in_db"]), conflict_options[0]["name"] if conflict_options else None)
                
                conflicts.append({
                    "id": str(uuid.uuid4()),
                    "options": conflict_options,
                    "suggested": suggested,
                    "reason": "Paziente già presente nel sistema - scegli quale nome usare" if has_existing else "Nomi simili trovati - verifica se sono lo stesso paziente",
                    "has_existing_patient": has_existing,
                    "type": "similar_names"
                })
            else:
                # NUOVA REGOLA: Nessun nome simile trovato, ma il paziente NON esiste nel DB
                # Chiedi comunque all'utente cosa fare: creare nuovo o sostituire
                processed_names.add(full_name)
                
                occ = name_occurrences.get(full_name, {"count": 0, "dates": set(), "tipo": set()})
                ...
                conflicts.append({
                    ...
                    "suggested": full_name,
                    "reason": "Nuovo paziente - scegli se creare o sostituire con esistente",
        ...
        return {
            ...
            "sync_id": current_sync_id,
            "sheets_processed": sheets_processed,
            "total_patients": len(new_patient_names),
